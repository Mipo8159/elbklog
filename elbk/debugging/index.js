...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({
      filename: "../elk-stack/microservice-logs/debugging.log",
    }),
  ],
});

const increaseTimestamp = (timestamp, ms) => new Date(timestamp.getTime() + ms);
const increaseTimestampRand = (timestamp, ms) =>
  increaseTimestamp(timestamp, Math.random() * ms);
...
function frontEnd() {
  let timestamp = new Date(
    new Date().getTime() - (5 + Math.random()) * FRONT_END_ACTION_DELAY_MS
  );
  ...
  try {
    ...
    if (err instanceof UserComplainedError) {
      console.log(
        `User ${err.userId} complained, charged but did not receive tickets`
      );
    }
  }
}
...
function searchEvents({ correlationId, timestamp, category }) {
  ...
  if (Math.random() < 0.01) {
    const err = new Error(
      "Error searching for events - database query timed out"
    );
    error({ timestamp, err });
    info({
      timestamp,
...
function getEvent({ correlationId, timestamp, eventId }) {
  ...
  if (Math.random() < 0.01) {
    const err = new Error(
      "Error fetching event details - database query timed out"
    );
    error({ timestamp, err });
    info({
      timestamp,
...
function purchaseTickets({
  ...
  info({
    timestamp,
    message: `Checking availability for ${numTickets} tickets`,
    eventId,
  });
  info({
    timestamp,
    message: "HTTP request",
    method: "GET",
    url: `/availability?eventId=${eventId}&numTickets=${numTickets}`,
    status: 200,
  });
  ...
  if (Math.random() < 0.8) {
    info({
      timestamp,
      message: `Reserved ${numTickets} tickets`,
      eventId,
      userId,
    });
    info({
      ...
      url: "/tickets/reserve",
      status: 200,
      body: { eventId, numTickets, userId },
    });
    timestamp = increaseTimestampRand(timestamp, TICKET_TIMEOUT_DELAY_MS);
    if (Math.random() < 0.7) {
      info({
        timestamp,
        message: `Purchased ${numTickets} tickets`,
        eventId,
        userId,
      });
      info({
      ...
      warn({
        timestamp,
        message: `Released hold on ${numTickets} tickets after timeout`,
        eventId,
        userId,
      });
  ...
  info({
    timestamp,
    message: "Creating purchase event",
    eventId,
    userId,
    numTickets,
  });
...
function createOrder({
  ...
  info({
    timestamp,
    message: "Processing purchase event",
    eventId,
    userId,
    numTickets,
  });
  if (Math.random() < 0.03) {
    const err = new Error(
      "Error creating order - could not hold lock on database table"
    );
    error({ timestamp, err });
    throw err;
  }
...
function processPayment({
  ...
  if (Math.random() < 0.03) {
    ...
    info({
      ...
      url: "/payment",
      status: 500,
      body: { orderId, amount, userId, eventId },
    });
    throw err;
  }
...
function sendNotification({
  ...
  if (Math.random() < 0.03) {
    const err = new UserComplainedError(
      "Order confirmation failed - Email service returned 429, too many requests error",
      userId
    );
